...
def main(page:ft.Page):
    ...
    def Download(e):
        ...
        for sect in section: #Create a Folder of the Section
            ...
            for mat in matieres: #Create a Folder of the "Matiere" inside the Section
                ...
                for year in range(*year_range): #Loop through the years (year).
                    for sess in sessions: #Loop through the sessions
                        ...
                        #option: all.
                        url = f'http://www.bacweb.tn/bac/{str(year)}/{sess}/math/{mat}.pdf' if \
                        (mat in ["philo","arabe"] and sect in ["technique","economie_gestion","sciences_ex","informatique"]) or \
                        (mat in ["francais","anglais"] and sect in ["economie_gestion","sciences_ex","informatique"]) or \
                        (mat =="info" and sect in ["technique","sciences_ex"]) or \
                        (mat == option_in.value) \
                        ...
                        if response.status_code == 200: #Success
                            success=True
                            with open(os.path.join(dir_path, f'{mat.upper()}-{year}-{sess[0].upper()}.pdf'), 'wb') as pdf: #Create the pdf.
    ...
    session_princiaple_in,session_controle_in = ft.Checkbox(label="Principale", value=True),ft.Checkbox(label="Controle", value=True)
    ...
    page.add(
        ft.Text("De l'année:"),
        ft.Slider(min=2009, max=2030, divisions=(21), label="{value}", on_change=slider_changedMin), year_min_in)
